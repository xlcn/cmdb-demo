...
class Asset(models.Model):
    ...
    business_unit = models.ForeignKey(to='BusinessUnit', on_delete=models.SET_NULL, null=True, blank=True, verbose_name='所属业务线')
    status = models.SmallIntegerField(choices=asset_status, default=0, verbose_name='设备状态')
    manufacture = models.ForeignKey(to="Manufacturer", null=True, blank=True, verbose_name='制造商', on_delete=models.SET_NULL)
    ...
    admin = models.ForeignKey(to=User, null=True, blank=True, verbose_name='资产管理员', related_name='admin', on_delete=models.SET_NULL)
...
class Server(models.Model):
    ...
    hosted_on = models.ForeignKey('self', null=True, blank=True, related_name='hosted_on_server', verbose_name='宿主机', on_delete=models.SET_NULL)
...
class SecurityDevice(models.Model):
    ...
    def __str__(self):
        return '{}--{} <id:{}>'.format(self.asset.name, self.get_sub_asset_type_display(), self.id)
...
class RAM(models.Model):
    ...
    def __str__(self):
        return '{}: {}: {}: {}'.format(self.asset.name, self.model, self.slot, self.capacity)
...
class EventLog(models.Model):
    """
    日志不能关联删除了，需要设置 on_delete = models.SET_NULL
    """
    event_type_choice = (
        (0, '其它'),
        (1, '硬件变更'),
    ...
    asset = models.ForeignKey('Asset', null=True, blank=True, on_delete=models.SET_NULL)  # 当资产审批通过之后 有这条数据
    ...
    user = models.ForeignKey(User, null=True, blank=True, verbose_name='事件执行人', on_delete=models.SET_NULL)  # 自动更新资产数据时没有执行人
