...
class ApproveAsset(object):
    ...
    def _server_upline(self):
        ...
        try:
            ...
            logger.exception('approve_failed, {} {} 审批失败'.format(self.new_asset.asset_type, self.new_asset.sn))
            return False
        else:
            log("upline", asset=asset, request=self.request)
            # 添加日志
            logger.info('upline, {} {} 服务上线'.format(self.new_asset.asset_type, self.new_asset.sn))
            return True
